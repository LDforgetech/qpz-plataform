...
function Sidebar({
  ...
  // O comportamento nativo mobile já é ser um overlay de gaveta que não altera o layout.
  if (isMobile) {
    return (
  ...
  return (
    <>
      {/* 
        Backdrop Escurecido para o modo Desktop Overlay: 
        Aparece quando aberto, intercepta o clique fora para fechar e evita interação na tela.
      */}
      <div
        className={cn(
          "fixed inset-0 z-40 bg-black/10 backdrop-blur-sm transition-all duration-200 hidden md:block",
          state === "expanded"
            ? "opacity-100 visible"
      ...
      <div
        className="group peer hidden text-sidebar-foreground md:block z-50 fixed"
        data-state={state}
      ...
      >
        {/* 
          Gap estático: 
          Agora mantemos a largura do "recuo" fixada no tamanho de ícone para nunca empurrar a tela. 
        */}
        <div
          data-slot="sidebar-gap"
          className={cn(
            "relative bg-transparent transition-none", // Removido o transition width que empurrava
        ...
        {/* 
          O contêiner aumenta o z-index para 50, ficando por cima de todo o resto do sistema.
        */}
        <div
          data-slot="sidebar-container"
          data-side={side}
          className={cn(
            "fixed inset-y-0 z-50 hidden h-svh w-[var(--sidebar-width)] transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex",
            variant === "floating" || variant === "inset"
